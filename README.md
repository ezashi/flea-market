...
| 下書き保存 | `/chat/:item_id/save-draft` | POST | 必要 |
| 下書き削除 | `/chat/:item_id/clear-draft` | POST | 必要 |
| メッセージ編集 | `/chat/message/:message_id/edit` | GET | 必要 |
| メッセージ更新 | `/chat/message/:message_id/update` | POST | 必要 |
| メッセージ削除 | `/chat/message/:message_id/delete` | POST | 必要 |
...
- **下書き保存**: チャット入力中のテキストを自動保存（2秒後に自動保存、他画面遷移時にも保持）
- **スマート並び替え**: 取引中商品を最新メッセージ順に自動並び替え
- **未読通知**: 未読メッセージ数をリアルタイム表示
- **取引完了**: 購入者による取引完了処理
- **評価機能**: 取引完了後の相互評価システム
...
- **購入履歴**: 購入した商品の一覧表示
- **出品履歴**: 出品した商品の一覧表示
- **取引中商品**: 現在取引中の商品とチャット機能（最新メッセージ順で表示）
